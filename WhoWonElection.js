...
function getWinner(ballots) {
  ...
  let mostVotes = Math.max(...values);
  let potentialWinner = keys.find(key => votes[key] === mostVotes);
  return mostVotes > ballots.length / 2 ? potentialWinner: null;
}