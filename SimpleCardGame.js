...
function winner(s, j) {
    ...
    let sPts = s.reduce((a,c,i) => cards.indexOf(s[i]) > cards.indexOf(j[i]) ? a+1 : a ,0);
    let jPts = s.reduce((a,c,i) => cards.indexOf(j[i]) > cards.indexOf(s[i]) ? a+1 : a ,0);
    return sPts === jPts ? 'Tie' : sPts > jPts ? `Steve wins ${sPts} to ${jPts}` : `Josh wins ${jPts} to ${sPts}`;
}