...
// The first parameter will contain the count of each race on the side of good in the 
// following order:
...
// All values are non-negative integers. The resulting sum of the worth for each side 
...
// eradicates all trace of Good" if evil wins, or "Battle Result: No victor on this 
...
function goodVsEvil(good, evil){
    ...
    for(let i = 0; i < goodScores.length; i++) {
      goodScore += goodScores[i] * good[i];      
    }
    for(let i = 0; i < evilScores.length; i++) {
      evilScore += evilScores[i] * evil[i];      
    }
